...
class employee { //4
    ...
    date data;
    etype dolj;
    public: employee(int ph=0, float sl=0, date dt, etype dl) {
        phone = ph;
        salary = sl;
    }

    void enterInfo(int ph=0, float sl=0, date dt, etype dl) {
        phone = ph;
        salary = sl;
        data = dt;
        dolj = dl;
...
Возможно, при создании методов вам понадобится ветвление switch для работы с перечисляемым типом etype. Напишите функцию main(), которая попросит пользователя ввести данные о трех сотрудниках, а затем выведет эти данные на экран.)"<<endl;
...
cout<<"Введите данные трёх сотрудников через энтер или пробел: ";
cin>>i>>f;
em1.enterInfo(i, f);
cin>>i>>f;
em2.enterInfo(i, f);
